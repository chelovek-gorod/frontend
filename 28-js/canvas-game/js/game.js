...
class Rocket extends Sprite {
    constructor(x, y, speed) {
        ...
        this.speed = speed;
        this.turnSpeed = 0.1;
        this.addSmokeSpeed = 30;
        this.newSmokeTimeout = this.addSmokeSpeed;
        this.isExist = true;
    ...
    update(dt) {
        ...
        // если нет цели - уничтожаем ракету
        if (!target) {
            this.isExist = false;
            player.rocketsReady++;
            //                class Explosion(x, y)
        ...
        // добавление дыма
        this.newSmokeTimeout -= dt;
        if (this.newSmokeTimeout <= 0) { this.newSmokeTimeout = this.addSmokeSpeed
            this.newSmokeTimeout += this.addSmokeSpeed;
            //            class Smoke(x, y)
            smokesArr.push( new Smoke(this.x, this.y));
        }
...
// Дым (от ракеты игрока)
class Smoke extends SpriteSheet {
    constructor(x, y) {
        // class SpriteSheet (imageName, x, y, fw, fh, frames, fps = 60)
        super('smoke_32x32px_25frames.png', x, y, 32, 32, 25, 15);
        this.isExist = true;
    }
...
// массивы дыма, взрывов и бонусов
let smokesArr = [];
let explosionsArr = [];
let bonusesArr = [];
...
// массивы вражеских кораблей, пуль и максимальное число вражеских кораблей при старте игры
let maxEnemies = 1;
let enemiesArr = [];
let enemiesBulletsArr = [];

// функция добавления нового вражеского корабля
function addEnemies() {
